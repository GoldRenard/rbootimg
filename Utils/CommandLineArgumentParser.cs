        ...
        public static String GetParamValue(string paramName)
        {
            if (requiredParameters != null)
            {
                if (requiredParameters.ContainsKey(paramName))
                    return requiredParameters[paramName].Trim().Length == 0 ? null : requiredParameters[paramName];
            }
            if (optionalParameters != null)
            {
                if (optionalParameters.ContainsKey(paramName))
                    return optionalParameters[paramName].Trim().Length == 0 ? null : optionalParameters[paramName];
            }
            return null;
        }

        /// <summary>
        /// Check is parameter specified.
        /// </summary>
        /// <param name="paramName">The name of the perameter.</param>
        /// <returns>Is specified.</returns>
        public static bool IsSpecified(string paramName)
        {
            if (GetParamValue(paramName) == null)
                return false;
            return GetParamValue(paramName).Trim().Length != 0;
        }
